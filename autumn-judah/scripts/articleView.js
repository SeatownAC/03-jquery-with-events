...
articleView.handleAuthorFilter = function() {
  $('#author-filter').on('change', function() {
    ...
    if ($(this).val()) {
      ...
      $('article').hide();
      $('article').filter(`[data-author="${currentAuthor}"]`).fadeIn();
    } else {
...
articleView.handleCategoryFilter = function() {
  ...
  $('#category-filter').on('change', function() {
    if ($(this).val()) {
      ...
      $('article').hide();
      $('article').filter(`[data-category="${currentCategory}"]`).fadeIn();
    } else {
      $('article').fadeIn();
      $('article.template').hide();
    }
...
// TODONE: Call all of the above functions, once we are sure the DOM is ready.*8*8*8
